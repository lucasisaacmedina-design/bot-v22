...
def get_modo_alfa(symbol="BTCUSDT"):
    try:
        ...
        if len(cierres) < 15:
            return "LATERAL LINEAL (0.30%)", {"name":"LOBO","tp":0.30,"sl":0.70,"cooldown":600,"emoji":"\U0001F43A"}, 0.3
        tr = [highs[i]-lows[i] for i in range(1,len(cierres))]
        atr = sum(tr[-14:])/14
        atr_pct = (atr / cierres[-1]) * 100
        if atr_pct < 0.25:
            return f"LATERAL LINEAL ({atr_pct:.2f}%)", {"name":"RATA SCALPER","tp":0.20,"sl":0.40,"cooldown":300,"emoji":"\U0001F400"}, atr_pct
        elif atr_pct < 0.60:
            return f"NORMAL ({atr_pct:.2f}%)", {"name":"LOBO","tp":0.30,"sl":0.70,"cooldown":600,"emoji":"\U0001F43A"}, atr_pct
        else:
            return f"EXPLOSIVO ({atr_pct:.2f}%)", {"name":"ALFA ASESINO","tp":0.90,"sl":0.50,"cooldown":0,"emoji":"\U0001F981"}, atr_pct
    except:
        return "NORMAL (0.30%)", {"name":"LOBO","tp":0.30,"sl":0.70,"cooldown":600,"emoji":"\U0001F43A"}, 0.3
...
<div>TP +{{ modo.tp }}% | SL -{{ modo.sl }}% | ATR {{ "%.2f"|format(atr) }}%</div>
</div>
...
def home():
    return render_template_string(
        ...
        mercado=estado.get("mercado","..."),
        modo=estado.get("modo",{"name":"LOBO","tp":0.30,"sl":0.70,"emoji":"\U0001F43A"}),
        atr=estado.get("atr",0),
                ...
                if p:
                    ...
                    if estado[s]["en_posicion"] and not en_pausa:
                        ...
                        if estado[s]["pnl"] >= modo["tp"]:
                            ...
                            estado["cuenta"]["ops"] += 1
                            msg = f"✅TP {modo['emoji']} +{modo['tp']}% {s} ${p:.2f} +${tp_neto:.2f} NETO Bal ${estado['cuenta']['balance']:.2f}"
                            tg(msg)
                        ...
                        if estado[s]["pnl"] <= -modo["sl"]:
                            ...
                            msg = f"❌SL {modo['emoji']} -{modo['sl']}% {s} ${p:.2f} -${sl_neto:.2f} NETO Bal ${estado['cuenta']['balance']:.2f}"
                            tg(msg)
                            estado["historial"].append(f"{time.strftime('%d/%m %H:%M')} {msg}")
                            estado[s]["entry"] = p
                            estado[s]["pnl"] = 0
