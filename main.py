...
print(f"### V26.8.3 FINAL - RESET DIARIO + AUTO ESTRATEGIA ###")
...
BIENVENIDA = """👋 MANADA V26.8.3 ADMIN FULL - MP PURO + API SEGURA 🐺
... tu texto bienvenida igual...
"""
...
def setapi_cmd(message):
    try:
        ...
        ud["api_key"] = api_key; ud["api_secret"] = api_secret; ud["api_cargada"] = True
        guardar_datos()
        if es_admin(message.chat.id):
            bot.send_message(message.chat.id, f"✅ API ADMIN CARGADA\nAPI: {api_key[:6]}...{api_key[-4:]}\nAhora BALANCE = ✅ ADMIN", reply_markup=get_menu_botones(True))
        else:
            bot.send_message(message.chat.id, f"✅ API CARGADA Lobo 🐺\nAPI: {api_key[:6]}...{api_key[-4:]}", reply_markup=get_menu_botones(False))
...
def prender(message):
    ...
    if not user_data.get("api_cargada") and not es_admin(message.chat.id):
        bot.send_message(message.chat.id,"⛔ Falta /setapi", reply_markup=get_menu_botones(False)); return
...
def balance(message):
    ...
    ganancia_hoy = ud["neto_hoy"]; balance_total = ud["balance"]
...
# --- resto de handlers pagar, quiero lobo, socios, callbacks, flask igual a tu base ---
@bot.message_handler(func=lambda m: m.text in ["💰 PAGAR", "/pagar"])
def pagar(message):
    dolar = DOLAR_CRIPTO['valor']
    ud = get_user_data(message.chat.id)
    api_status = "✅ CARGADA" if ud.get("api_cargada") else "❌ FALTA /setapi"
    txt = f"""💰 PAGAR V26.8.3 - MP PURO + API SEGURA
...
def quiero_lobo(message):
    ...
    if not ud.get("api_cargada"):
        bot.send_message(message.chat.id, f"⛔ Primero /setapi", reply_markup=get_menu_botones(es_admin(message.chat.id))); return
    markup = types.InlineKeyboardMarkup(row_width=1)
    markup.add(types.InlineKeyboardButton(f"🐀 RATA $20/mes (${20*dolar} ARS)", callback_data="plan_RATA"), types.InlineKeyboardButton(f"🐺 LOBO $40/mes (${40*dolar} ARS)", callback_data="plan_LOBO"), types.InlineKeyboardButton(f"🦈 TIBURON $60/mes (${60*dolar} ARS)", callback_data="plan_TIBURON"))
    bot.send_message(message.chat.id, f"🐺 ELEGÍ TU MODO\nDolar: ${dolar}\nAlias: {ALIAS_MP_DEMO}", reply_markup=markup)

@bot.callback_query_handler(func=lambda call: call.data.startswith("plan_"))
def callback_plan(call):
    plan = call.data.split("_")[1]; dolar = DOLAR_CRIPTO['valor']; precio_usd = PLANES[plan]
    bot.answer_callback_query(call.id, f"Elegiste {plan}")
    bot.send_message(call.message.chat.id, f"✅ Elegiste {plan}\n💰 A pagar: ${precio_usd} USD = ${precio_usd*dolar} ARS\nAlias MP: {ALIAS_MP_DEMO}\nEnviá comprobante con /comprobante")

def enviar_lista_socios(chat_id):
    if not ESTADO["socios"]:
        bot.send_message(chat_id, "👥 Sin socios aun", reply_markup=get_menu_botones(True)); return
    ...
    for cid,d in list(ESTADO["socios"].items()):
        dias=(d["vence"]-datetime.now()).days; horas=int((d["vence"]-datetime.now()).total_seconds()//3600)%24
        if dias < 0: dias = 0
        u = USUARIOS.get(cid, {"balance":200,"ops_hoy":0,"neto_hoy":0,"ganadas":0,"perdidas":0})
        win = calcular_winrate(u)
        markup = types.InlineKeyboardMarkup(row_width=3)
        markup.add(types.KeyboardButton("👁️ VER CAJA") if False else types.InlineKeyboardButton("👁️ VER CAJA", callback_data=f"ver_{cid}"), types.InlineKeyboardButton("➕ +30D", callback_data=f"add30_{cid}"), types.InlineKeyboardButton("❌ BAJA", callback_data=f"baja_{cid}"))
        txt = f"👤 {cid}\n📦 {d['plan']} - ⏳ {dias}d {horas}h\n💰 ${u['balance']} | 📈 ${u['neto_hoy']} | 🎯 {win}%\n🔗 {WEB_URL}/?id={cid}"
...
def callback_gestion_socios(call):
    if not es_admin(call.message.chat.id): bot.answer_callback_query(call.id, "Solo admin"); return
    try:
        if call.data.startswith("ver_"):
            cid = int(call.data.split("_")[1]); bot.answer_callback_query(call.id, f"Viendo {cid}"); bot.send_message(call.message.chat.id, f"🔗 Caja {cid}:\n{WEB_URL}/?id={cid}")
        elif call.data.startswith("add30_"):
            cid = int(call.data.split("_")[1])
            if cid in ESTADO["socios"]:
                ESTADO["socios"][cid]["vence"] += timedelta(days=30); guardar_datos()
                dias = (ESTADO["socios"][cid]["vence"] - datetime.now()).days
                bot.answer_callback_query(call.id, f"+30 OK - {dias}d"); bot.send_message(call.message.chat.id, f"✅ {cid} +30D -> {dias} dias")
        elif call.data.startswith("baja_"):
            cid = int(call.data.split("_")[1])
            markup = types.InlineKeyboardMarkup(); markup.add(types.InlineKeyboardButton("✅ SI, BAJA", callback_data=f"confirm_baja_{cid}"), types.InlineKeyboardButton("❌ CANCELAR", callback_data="cancel_baja"))
            bot.send_message(call.message.chat.id, f"⚠️ Confirmar baja de {cid}?", reply_markup=markup); bot.answer_callback_query(call.id, "Confirma baja")
        elif call.data.startswith("confirm_baja_"):
            cid = int(call.data.split("_")[2])
            if cid in ESTADO["socios"]: del ESTADO["socios"][cid]; guardar_datos(); bot.answer_callback_query(call.id, f"Baja OK"); bot.send_message(call.message.chat.id, f"❌ Baja {cid} ejecutada")
...
def cancel_baja(call): bot.answer_callback_query(call.id, "Cancelado"); bot.delete_message(call.message.chat.id, call.message.message_id)

HTML="""<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>V26.8.3</title><script src="https://s3.tradingview.com/tv.js"></script><style>body{margin:0;background:#131722;color:#d1d4dc;font-family:Arial}.header{background:#1e222d;padding:10px}.box{padding:12px;margin:6px;border-radius:10px;font-size:13px;line-height:1.7}.admin{background:#0d2a4a;border-left:5px solid #00ffea}.socio{background:#2a2218;border-left:5px solid #ff9800}.kpi{display:inline-block;background:#1e222d;padding:7px 10px;border-radius:6px;margin:3px;font-size:12px;border:1px solid #2a2e39}#chart_btc{height:50vh}#chart_bnb{height:30vh}.btn{display:inline-block;margin-top:8px;padding:8px 14px;background:#00ffea;color:#000;border-radius:6px;text-decoration:none;font-weight:bold}</style></head><body><div class="header"><b id="titulo">V26.8.3 - RESET DIARIO</b><div id="admin" class="box admin">Cargando...</div><div id="socios" class="box socio">Cargando socios...</div></div><div id="chart_btc"></div><div id="chart_bnb"></div><script>new TradingView.widget({"autosize":true,"symbol":"BINANCE:BTCUSDT","interval":"1","theme":"dark","container_id":"chart_btc"});new TradingView.widget({"autosize":true,"symbol":"BINANCE:BNBUSDT","interval":"1","theme":"dark","container_id":"chart_bnb"});function getId(){return new URLSearchParams(window.location.search).get('id')}async function r(){let sid=getId();if(sid){try{let d=await (await fetch('/api/socio/'+sid)).json();if(d.error){document.getElementById('admin').innerHTML='⛔ No existe<br><a class="btn" href="/">VOLVER</a>';document.getElementById('socios').style.display='none';return}document.getElementById('titulo').innerHTML=`🟠 CAJA SOCIO ${sid} - ${d.plan}`;document.getElementById('admin').innerHTML=`🟠 CAJA CACHORRO 20%<br><span class="kpi">💰 $${d.balance}</span><span class="kpi">📈 $${d.neto_hoy}</span><span class="kpi">🔄 ${d.ops_hoy}</span><span class="kpi">🎯 ${d.winrate}%</span><br><span class="kpi">⚙️ ${d.modo}</span><span class="kpi">📊 ${d.mercado}</span><br><span class="kpi">📦 ${d.plan} - ⏳ ${d.vence_dias}d</span><br><span class="kpi">₿ BTC: $${d.btc}</span><span class="kpi">🔶 BNB: $${d.bnb}</span>`;document.getElementById('socios').style.display='none';}catch(e){document.getElementById('admin').innerHTML='Error'}}else{let a=await (await fetch('/api/data')).json();document.getElementById('admin').innerHTML=`🔵 ADMIN<br><span class="kpi">💰 $${a.balance}</span><span class="kpi">📈 $${a.neto_hoy}</span><span class="kpi">🔄 ${a.ops_hoy}</span><span class="kpi">🎯 ${a.winrate}%</span><br><span class="kpi">⚙️ ${a.modo}</span><span class="kpi">📊 ${a.mercado}</span><br><span class="kpi">₿ $${a.btc}</span><span class="kpi">🔶 $${a.bnb}</span><span class="kpi">💵 $${a.dolar.valor}</span>`;let s=await (await fetch('/api/socios')).json();let h='🟠 CAJAS SOCIOS:<br>';for(let k in s.socios){let u=s.socios[k];h+=`<div style="margin:8px 0;padding:8px;background:#1e222d;border-radius:8px">👤 ${k} - ${u.plan} | 💰 $${u.balance} | 📈 $${u.neto_hoy} | 🔄 ${u.ops} ops | 🎯 ${u.winrate}% - ⏳ ${u.vence_dias}d<br><a class="btn" href="/?id=${k}">VER CAJA</a></div>`;}if(Object.keys(s.socios).length==0)h+='Sin socios';document.getElementById('socios').innerHTML=h;}}setInterval(r,3000);r();</script></body></html>"""
