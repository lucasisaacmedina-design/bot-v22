...
def detectar_BI_CEREBRO(regimen):
    ...
    for sym in MONEDAS_ACTIVAS:
        has_tiburon = any(p["symbol"]==sym and p["estrategia"]=="TIBURON" for pos_list in POSICIONES_ABIERTAS.values() for p in (pos_list if isinstance(pos_list, list) else []))
        for nombre in todas:
            if nombre=="TIBURON" and has_tiburon: continue
...
def motor_v45():
    print(">>> MOTOR V45 BI-CEREBRO + TANQUE BNB + RECUPERACION 3%")
    time.sleep(5)
    while True:
        try:
        ...
        for user_id in list(USUARIOS.keys()):
            ...
            if ok and estrategia_elegida:
                if estrategia_elegida in ["TIBURON","KRAKEN"]:
                    ya_tiene = any(p["symbol"]==symbol_elegido and p["estrategia"]==estrategia_elegida for p in POSICIONES_ABIERTAS[user_id])
                ...
                if exito:
                    ...
                    try: bot.send_message(user_id,f"🟢 V45 {estrategia_elegida}{tipo_banda} {symbol_elegido}\n{motivo}\nEnt {precio:.2f} TP {tp_inteligente}% SL {cfg['sl_neto']}% Neto {neto:.2f}%\n📊 {link}")
                    except: pass
        guardar_datos()
        time.sleep(60)
...
def start(m):
    ...
    estado_txt = "🟢 V45" if u["prendido"] else "🔴 APAGADO"
    ...
    bot.send_message(m.chat.id,f"🦁 V45 BI-CEREBRO {estado_txt}\n{regs}\n{bandas_txt}\n{'+'.join(MONEDAS_ACTIVAS)}\nBal ${u['balance']:.2f}\n{WEB_URL}",reply_markup=get_menu())
...
def chart_page():
    ...
    html = f"""<!DOCTYPE html><html><head><meta charset="utf-8"><title>{symbol}</title><script src="https://s3.tradingview.com/tv.js"></script></head><body style="margin:0;background:#0f1115"><div style="padding:10px;background:#1e222d;color:#fff">{symbol} - V45 <a href="/" style="color:#facc15">Volver</a></div><div id="chart" style="height:90vh"></div><script>new TradingView.widget({{"autosize":true,"symbol":"BINANCE:{symbol}","interval":"{interval}","timezone":"America/Argentina/Buenos_Aires","theme":"dark","container_id":"chart"}});</script></body></html>"""
    return render_template_string(html)
