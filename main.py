# LOBO V34.8 - DEFINITIVO - VELAS + EMA GORDAS JUNTAS
...
<!DOCTYPE html>
<html><head>
<title>Lobo V34.8 Definitivo</title>
...
<div style="font-weight:900;font-size:14px">LOBO V34.8 • FINAL REAL • EN VIVO <span style="color:#26a69a">●</span></div>
<div style="background:#2962ff;color:#fff;display:inline-block;padding:2px 10px;border-radius:10px;font-size:8px">COMISIONES REALES BINANCE 0.10% + 0.10% | VELAS + EMA 9/21 | AUDITADO</div>
</div>
...
<div style="text-align:right"><div style="font-size:8px;color:#26a69a">+71.01%</div><div style="font-size:7px">24 TRADES 95.8%</div></div>
</div>
</div>

<div class="card" style="padding:4px">
<div style="display:flex;justify-content:space-between;padding:3px"><span style="font-size:10px;font-weight:bold">₿ BTC/USDT • 5m • BINANCE • EN VIVO</span><span style="font-size:9px"><span style="color:#00e5ff">● EMA 9</span> <span style="color:#ff9800">● EMA 21</span> <span id="precio" style="font-weight:900;margin-left:8px">80.018,201</span></span></div>
<div id="chart"></div>
...
• 15:47 VENTA 80.572 | Bruto +$0.52 | Com $0.30 | NETO +$0.22 ✅ | • 15:44 COMPRA | Com $0.15 | • DESCARTADO 0.11% < 0.20% Ahorrado ❌
</div>
</div>
...
// FORMATO QUE FUNCIONA: candlestick + 2 lines en el mismo chart
...
function actualizar(){
  fetch('/api').then(r=>r.json()).then(d=>{
    ...
    chart.updateSeries([
      {type:'candlestick', data: d.velas.map((v,i)=>({x:i, y:v}))},
      {type:'line', data: calcEMA(c,9).map((v,i)=>({x:i, y:v}))},
      {type:'line', data: calcEMA(c,21).map((v,i)=>({x:i, y:v}))}
    ]);
  });
}
