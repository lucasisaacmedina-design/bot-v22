...
ESTADO = "COMPRADO"
HISTORIAL = ["13:24:01 COMPRA $152753","13:23:37 VENTA +0.80% | +$1.20","13:09:06 VENTA +0.86% | +$1.15","13:02:34 VENTA +0.81% | +$1.10"]
    ...
    while True:
        try:
            ...
            if ESTADO=="COMPRADO":
                ...
                if g>=0.008:
                    PNL+=1.2; TRADES+=1; HISTORIAL.insert(0,f"{datetime.now().strftime('%H:%M:%S')} VENTA +{g*100:.2f}%")
...
#chart{{width:100%;height:220px;border-radius:12px;overflow:hidden;background:#fff;margin:10px 0;border:1px solid #eee}}
.bar{{background:#eee;height:8px;border-radius:10px;overflow:hidden}}.fill{{height:100%;transition:width 0.5s}}
</style></head><body>
<div class="card"><div style="text-align:center"><div style="font-size:12px;color:#666">PROYECTO FAMILIA LOBO</div><div style="font-weight:800">LOBO V33 - VELAS PRO</div></div>
...
<div class="row"><span class="sub">Inicial: ${USDT_INICIAL}</span><span style="color:#00c853;font-weight:bold">+${PNL:.2f}</span></div>
</div>
<div id="chart"></div>
<div class="row"><span class="sub">Operación</span><span class="badge" style="background:{bg};color:{color}">{badge}</span></div>
...
<div class="sub" style="text-align:center;margin-top:4px">Vende en +0.80% | {TRADES} trades</div>
<div style="margin-top:12px"><div class="sub" style="font-weight:bold">📋 Últimos movimientos</div><div style="font-size:12px;color:#555;line-height:1.6">{"<br>".join(HISTORIAL)}</div></div>
<div style="text-align:center;font-size:11px;color:#999;margin-top:10px">Gráfico BTC/USDT en vivo • Bot 24/7 • Bella Vista</div>
</div>
<script>
...
const candleSeries=chart.addCandlestickSeries(); let last={os.time()*1000};
fetch('https://api.binance.com/api/v3/klines?symbol=BTCUSDT&interval=1m&limit=50').then(r=>r.json()).then(data=>{{
 ...
 let base={PRECIO_ACTUAL}; let data=[]; for(let i=50;i>0;i--){{let o=base+(Math.random()-0.5)*200; let c=o+(Math.random()-0.5)*300; data.push({{time:Math.floor(Date.now()/1000)-i*60,open:o,high:Math.max(o,c)+100,low:Math.min(o,c)-100,close:c}});}}
