...
def mandar_pensamiento_telegram():
    ...
    try:
        for uid in list(USUARIOS.keys()):
            ...
            if lineas:
                texto = f"🐺 V50.6.4.2 KRAKEN LIBRE 1 TIRO\n" + "\n".join(lineas) + f"\n📊 {WEB_URL}"
...
def detectar_BI_CEREBRO(regimen):
    ...
    for sym in MONEDAS_ACTIVAS:
        ...
        for nombre in orden:
            ...
            if nombre!= "KRAKEN" and counts_global.get((sym,nombre),0) >= 1: continue
            if nombre == "KRAKEN" and kraken_en_sym >= 1: continue
            if nombre!= "KRAKEN" and not adentro_normal and total_en_sym >= 1: continue
    ...
    return False, f"V50.6.4.2 KRAKEN LIBRE - ACECHANDO", MONEDAS_ACTIVAS[0], None, 0
...
def motor_v45():
    ...
    print(f">>> MOTOR V50.6.4.2 KRAKEN LIBRE 1 TIRO {len(MONEDAS_ACTIVAS)}/{MAX_MONEDAS}")
    time.sleep(5)
    while True:
        try:
...
def start(m):
    ...
    estado_txt = f"🟢 V50.6.4.2 KRAKEN LIBRE {len(MONEDAS_ACTIVAS)}/{MAX_MONEDAS}" if u["prendido"] else "🔴 APAGADO"
...
def balance(m):
    ...
    bandas_txt = "\n".join([banda_txt_display(k,v) for k,v in BANDAS_ACTIVAS.items() if v.get("activa")]) or "Sin bandas"
    bot.send_message(m.chat.id,f"💰 V50.6.4.2 KRAKEN LIBRE {len(MONEDAS_ACTIVAS)}/{MAX_MONEDAS}\n{regs}\n{bandas_txt}\n{pos_txt}\nBal ${u['balance']:.2f} Total ${ganancia_total:+.2f}\nHoy ${u['neto_hoy']:+.2f}",reply_markup=get_menu())

@bot.message_handler(func=lambda m: m.text in ["📜 HISTORIAL","/historial"])
def historial(m):
    u=get_user_data(m.chat.id)
    txt="\n".join(u["historial"][-25:]) if u["historial"] else "Sin ops"
    bot.send_message(m.chat.id,f"📜 V50.6.4.2 KRAKEN LIBRE\n{txt}",reply_markup=get_menu())

@bot.message_handler(func=lambda m: m.text in ["🚀 PRENDER","/prender"])
def prender(m):
    u=get_user_data(m.chat.id); u["prendido"]=True; u["modo"]=f"CAZANDO KRAKEN LIBRE"
    guardar_datos()
    bot.send_message(m.chat.id,f"🐺 LOBO AL ATAQUE! V50.6.4.2 KRAKEN LIBRE 1 TIRO PRENDIDO - Manada cazando con RSI27 aunque tenga TIBURON+LOBO",reply_markup=get_menu())

@bot.message_handler(func=lambda m: m.text in ["⏸️ APAGAR","/apagar"])
def apagar(m):
    u=get_user_data(m.chat.id); u["prendido"]=False; guardar_datos()
    bot.send_message(m.chat.id,f"⏸️ V50.6.4.2 APAGADO",reply_markup=get_menu())
...
def ordenes_estado(m):
    ...
    if not lista:
        ...
        return
    ganancia_total = u["balance"]-u["capital_inicial"]+BOLSA_PIRANA["neto"]+BOLSA_PIRANA_NEGRA["neto"]
    txt = f"📦 MANADA V50.6.4.2 - {len(lista)} POS ABIERTAS\n"
    for p in lista:
