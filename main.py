# LOBO V32.1 - PROLIJO EDITION - PARA TU HERMANO
    ...
    while True:
        try:
            ...
            if ESTADO == "COMPRADO":
                ...
                if ganancia >= 0.008:
                    ganancia_usd = PRECIO_ACTUAL * 0.001 * 0.008
                    PNL += ganancia_usd
                    USDT = USDT_INICIAL + PNL + (ganancia * 100)  # fix simple
                    # Correccion final: USDT siempre = inicial + PnL + flotante
                    USDT = USDT_INICIAL + PNL + (PRECIO_ACTUAL - PRECIO_COMPRA) * 0.001
                    TRADES += 1
                    hora = datetime.now().strftime("%H:%M:%S")
                    HISTORIAL.insert(0, f"{hora} VENTA +{ganancia*100:.2f}% | Ganancia +${ganancia_usd:.2f}")
                    HISTORIAL = HISTORIAL[:5]
                    ESTADO = "VENDIDO"
                    USDT = USDT_INICIAL + PNL # al vender, cuadra perfecto
            else:
...
def home():
    ...
    estado_txt = f"COMPRADO ${PRECIO_COMPRA:.0f} | Ahora {ganancia_actual:+.2f}% | Vende en +0.8%" if ESTADO=="COMPRADO" else "VENDIDO - Buscando compra"
    ...
    .box{{border:2px solid #00ff00;padding:15px;border-radius:10px;max-width:420px;margin:auto}}
    .demo{{background:yellow;color:black;padding:6px;text-align:center;font-weight:bold;font-size:13px}}
    </style></head><body>
    <div class="box">
    <div class="demo">MODO DEMO - SIN DINERO REAL - PROYECTO FAMILIA LOBO</div>
    <h2>LOBO V32.1 - RENDER 24/7</h2>
    USDT: ${usdt_mostrado:.2f} (Inicial ${USDT_INICIAL})<br>
    BTC: {BTC:.6f}<br>
    ...
    {estado_txt}<br><br>
    <small>Bot corriendo aunque apagues tu compu - 24/7<br>
    Estrategia propia - Sin terceros</small><br><br>
    {"<br>".join(HISTORIAL)}
    </div></body></html>
    """
