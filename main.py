...
ESTADO = {
    ...
    "btc_inicial": 100.0,"bnb_inicial": 100.0,"ops_hoy": 0,"neto_hoy": 0.0,
    "ganadas": 0,"perdidas": 0,"modo": "CACHORRO","mercado": "CACHORRO GRATIS 7 DIAS (20%)",
    "pausa_hasta": None,"btc": 78287.4,"bnb": 739.68,"historial": [],
    "btc_history": [78287.4 + random.uniform(-200,200) for _ in range(30)],
    "socios": {},"admins": ADMINS_IDS
}
...
def motor_demo():
    while True:
        ...
        if ESTADO["modo"]!="CACHORRO": analizar_mercado_y_elegir_modo()
        ESTADO["btc_history"].append(ESTADO["btc"])
        if len(ESTADO["btc_history"])>30: ESTADO["btc_history"]=ESTADO["btc_history"][-30:]
        modo=ESTADO["modo"]; factor=CACHORRO_PORC if modo=="CACHORRO" else 1.0
        ...
        if es_ganada:
            ESTADO["ganadas"]+=1; ESTADO["ops_hoy"]+=1; ESTADO["neto_hoy"]=round(ESTADO["neto_hoy"]+gan,2); ESTADO["balance"]=round(ESTADO["balance"]+gan,2)
            ESTADO["historial"].append(f"{datetime.now().strftime('%H:%M')} - BTC - {modo} - TP {tp} = +${gan} Neto")
        else:
            ESTADO["perdidas"]+=1; ESTADO["ops_hoy"]+=1; ESTADO["neto_hoy"]=round(ESTADO["neto_hoy"]-perd,2); ESTADO["balance"]=round(ESTADO["balance"]-perd,2)
...
def baja(message):
    ...
    try:
        ...
        if idc in ESTADO["socios"]: del ESTADO["socios"][idc]; bot.send_message(message.chat.id,f"🗑️ Baja OK ID {idc} eliminado");
        else: bot.send_message(message.chat.id,"ID no encontrado")
...
def prender(message):
    ...
    ESTADO["prendido"]=True; ESTADO["pausa_hasta"]=None; ESTADO["modo"]="CACHORRO"; ESTADO["mercado"]="CACHORRO GRATIS 7 DIAS (20%)"
    # FIX 999 DIAS
...
def balance(message):
    ...
    win=calcular_winrate(); btc_part=ESTADO["balance"]*0.5; bnb_part=ESTADO["balance"]*0.5
...
def historial(message):
    hist="\n".join(ESTADO["historial"][-15:]) or "Sin ops aún"
