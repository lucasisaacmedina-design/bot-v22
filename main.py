...
def responder(m):
    ...
    elif "$10" in txt: bot.send_message(m.chat.id, f"Con $10 arrancás. Yo estoy operando ahora mismo con ${balance:.2f} y mi neta de hoy es +${ganancia_bruta-comision_acum:.2f}. Mirá en vivo: https://bot-v22.onrender.com")
    elif "vela" in txt: bot.send_message(m.chat.id, "Una vela = la batalla de 5 min entre compradores y vendedores.")
...
def loop_telegram():
    print("Telegram V43 ON - Anti-conflicto")
    while True:
        try:
            bot.infinity_polling(timeout=10, long_polling_timeout=5, skip_pending=True)
        except Exception as e:
            print(f"Conflicto detectado, reintentando en 5 seg: {e}")
            time.sleep(5)
