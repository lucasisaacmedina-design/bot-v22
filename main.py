...
print(f"### V25.4 FULL ADMIN + RETORNO - DISCO: {DATA_FILE} ###")
...
def get_user_data(user_id):
    ...
    if user_id not in USUARIOS:
        USUARIOS[user_id] = {"prendido": False,"balance": BALANCE_INICIAL,"balance_inicial": BALANCE_INICIAL,"btc_inicial": 100.0,"bnb_inicial": 100.0,"ops_hoy": 0,"neto_hoy": 0.0,"ganadas": 0,"perdidas": 0,"modo": "CACHORRO","mercado": "CACHORRO GRATIS 7 DIAS (20%)","pausa_hasta": None,"historial": [],"caja": "ADMIN" if user_id in ADMINS_IDS else "SOCIO"}
        guardar_datos()
...
def analizar_mercado_y_elegir_modo(user_data):
    if user_data["modo"]=="CACHORRO": user_data["mercado"]="CACHORRO GRATIS 7 DIAS (20%)"; return 0.20
    try:
...
def alta(message):
    ...
    try:
        ...
        try: bot.send_message(id_cliente,f"🐺 ¡Alta! Plan {plan} {dias}d\n👉 Tu web privada (solo tu caja):\n{WEB_URL}/?id={id_cliente}\n/Prender")
        except: bot.send_message(message.chat.id,f"⚠️ Socio no habló con bot. Pasale vos: {WEB_URL}/?id={id_cliente}")
...
def reset_user(message):
    ...
    try:
        ...
        user_data = get_user_data(idc); user_data["balance"]=monto; user_data["balance_inicial"]=monto; user_data["btc_inicial"]=monto/2; user_data["bnb_inicial"]=monto/2; user_data["neto_hoy"]=0.0; user_data["ops_hoy"]=0; user_data["ganadas"]=0; user_data["perdidas"]=0; user_data["historial"]=[]; user_data["pausa_hasta"]=None; guardar_datos()
        bot.send_message(message.chat.id,f"♻️ RESET OK {idc} -> ${monto}")
...
def add_balance(message):
    if not es_admin(message.chat.id): return
    try:
        parts=message.text.split(); idc=int(parts[1]); monto=float(parts[2]); user_data=get_user_data(idc); user_data["balance"]=round(user_data["balance"]+monto,2); guardar_datos(); bot.send_message(message.chat.id,f"➕ ${monto} a {idc} -> ${user_data['balance']}")
...
def baja(message):
    ...
    try:
        ...
        guardar_datos(); bot.send_message(message.chat.id,f"🗑️ Baja OK {idc}")
...
def start(message):
    ...
    if not acceso and len(ESTADO["socios"])>0 and not es_admin(message.chat.id): bot.send_message(message.chat.id,f"🔒 Privado ID: {message.chat.id}"); return
    if es_admin(message.chat.id): bot.send_message(message.chat.id,f"👋 V25.4 ADMIN 🐺\nTu web: {WEB_URL}\n/Prender /socios")
    else: bot.send_message(message.chat.id,f"👋 V25.4 🐺\nPlan: {ESTADO['socios'][message.chat.id]['plan']}\nTu web:\n{WEB_URL}/?id={message.chat.id}\n/Prender")

@bot.message_handler(commands=['Prender','prender'])
def prender(message):
    acceso,dias_rest=tiene_acceso(message.chat.id)
    if not acceso: bot.send_message(message.chat.id,"⛔ Vencido"); return
    user_data = get_user_data(message.chat.id); user_data["prendido"]=True; user_data["pausa_hasta"]=None; user_data["modo"]="CACHORRO"; user_data["mercado"]="CACHORRO GRATIS 7 DIAS (20%)"; guardar_datos()
    link = f"{WEB_URL}/?id={message.chat.id}" if not es_admin(message.chat.id) else WEB_URL
    bot.send_message(message.chat.id,f"🚀 ACTIVA ${user_data['balance']}\n{link}")

@bot.message_handler(commands=['balance'])
def balance(message):
    acceso,dias_rest=tiene_acceso(message.chat.id)
    if not acceso: return
    user_data = get_user_data(message.chat.id); win=calcular_winrate(user_data)
    bot.send_message(message.chat.id,f"💰 {user_data['caja']}\nBal: ${user_data['balance']} | Neto: ${user_data['neto_hoy']} | Ops: {user_data['ops_hoy']} | Win: {win}%\nModo: {user_data['modo']} | {user_data['mercado']}\nEstado: {get_estado_texto(user_data)}\nBTC ${ESTADO['btc']} BNB ${ESTADO['bnb']}")
...
def apagar(message):
    ...
    bot.send_message(message.chat.id,f"🛑 {user_data['caja']} pausada ${user_data['balance']}",reply_markup=markup)

@bot.callback_query_handler(func=lambda c: True)
def callbacks(c):
    user_data = get_user_data(c.message.chat.id)
    if c.data=="retirar": bot.send_message(c.message.chat.id,f"💸 {user_data['caja']}: ${user_data['balance']}")
    elif c.data=="reanudar": user_data["prendido"]=True; guardar_datos(); bot.send_message(c.message.chat.id,"▶️ Reanudado")

# ================= HTML V25.4 FULL CON RETORNO =================
HTML="""<!DOCTYPE html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>V25.4 FULL</title><script src="https://s3.tradingview.com/tv.js"></script><style>body{margin:0;background:#131722;color:#d1d4dc;font-family:Arial}.header{background:#1e222d;padding:10px}.box{padding:12px;margin:6px;border-radius:10px;font-size:13px;line-height:1.6}.admin{background:#0d2a4a;border-left:5px solid #00bfff}.socio{background:#3a2a1a;border-left:5px solid #ff9800}.kpi{display:inline-block;background:#1e222d;padding:5px 8px;border-radius:6px;margin:3px;font-size:12px;border:1px solid #2a2e39}.btn{display:inline-block;margin-top:10px;padding:9px 16px;background:#00bfff;color:#000;border-radius:8px;text-decoration:none;font-weight:bold}.btn2{display:inline-block;margin-left:6px;padding:9px 16px;background:#2a2e39;color:#fff;border-radius:8px;text-decoration:none}#chart_btc{height:50vh}#chart_bnb{height:28vh}a{color:#00bfff}</style></head><body><div class="header"><b id="titulo">🐺 V25.4 CAJAS SEPARADAS FULL</b><div id="admin" class="box admin">Cargando ADMIN FULL...</div><div id="socios" class="box socio">Cargando SOCIOS...</div><div id="infoPlan" class="box socio" style="display:none"></div></div><div id="chart_btc"></div><div id="chart_bnb"></div>
<script>
...
async function r(){
 ...
 if(sid){
   try{
     ...
     if(d.error){document.getElementById('admin').innerHTML='⛔ No existe<br><a class="btn" href="/">⬅️ Volver a mi caja ADMIN</a>';return}
     ...
     document.getElementById('admin').innerHTML=`🟠 TU CAJA - PLAN ${d.plan}<br>
        ...
        <a class="btn" href="/">⬅️ Volver a mi caja ADMIN</a>`;
     ...
     document.getElementById('infoPlan').innerHTML=`📋 PLAN: ${d.plan} | TP/SL: ${tp}<br>Link privado:?id=${sid}`;
   }catch(e){document.getElementById('admin').innerHTML='Error<br><a class="btn" href="/">⬅️ Volver</a>'}
 }else{
   let a=await (await fetch('/api/data')).json();
   document.getElementById('admin').innerHTML=`🔵 CAJA ADMIN 100% - TU PARTE SEPARADA (No toca socios)<br>
      ...
      Estado: ${a.estado_texto} | Disco: ${a.disco}`;
   let s=await (await fetch('/api/socios')).json();
   let h='🟠 CAJAS SOCIOS 20% SEPARADAS:<br>';
   for(let k in s.socios){
     ...
     <a class="btn" href="/?id=${k}">➡️ Ver caja privada?id=${k}</a></div>`;
   }
...
def api_socios():
    ...
    for cid,d in ESTADO["socios"].items():
        ...
        out[cid]={"balance":u["balance"],"ops":u["ops_hoy"],"estado":get_estado_texto(u),"plan":d["plan"],"neto_hoy":u["neto_hoy"],"winrate":calcular_winrate(u),"modo":u["modo"],"mercado":u["mercado"]}
...
def api_socio_individual(socio_id):
    ...
    sd=ESTADO["socios"][socio_id]; u=USUARIOS.get(socio_id) or get_user_data(socio_id); dias=(sd["vence"]-datetime.now()).days+1
    return jsonify({"balance":u["balance"],"neto_hoy":u["neto_hoy"],"ops_hoy":u["ops_hoy"],"winrate":calcular_winrate(u),"modo":u["modo"],"mercado":u["mercado"],"estado_texto":get_estado_texto(u),"plan":sd["plan"],"vence_dias":dias,"vence":sd["vence"].strftime("%d/%m/%Y"),"id":socio_id,"btc":ESTADO["btc"],"bnb":ESTADO["bnb"]})
